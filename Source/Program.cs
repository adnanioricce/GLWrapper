...
namespace GLWrapper
{
    public class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var vertices = GetCubeData().Select(v => new ColoredVertex(v.Position,new Color4(v.Color.R * (float)rng.NextDouble(),v.Color.G * (float)rng.NextDouble(),v.Color.B * (float)rng.NextDouble(),v.Color.A)))
                                        .ToArray();
            var translations = new List<Vector3>();
            for(int x = -10;x < 10; ++x) {
                for (int y = -10; y < 10; ++y) {
                    for (int z = -10; z < 10; ++z) {
                        var vector = new Vector3((float)x / 10.0f + offset, (float)y / 10.0f + offset, (float)z / 10.0f + offset);
